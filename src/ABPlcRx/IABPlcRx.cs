...
public interface IABPlcRx : IDisposable
{
    ...
    void AddUpdateTagItem<T>(string tagName, T? typeWitness = default);
    ...
    /// <param name="typeWitness">Optional type witness for callers that infer <typeparamref name="T"/> from a value.</param>
    /// <exception cref="System.ArgumentNullException">tagName.</exception>
    void AddUpdateTagItem<T>(string variable, string tagName, T? typeWitness = default);
    ...
    /// <param name="typeWitness">Optional type witness for callers that infer <typeparamref name="T"/> from a value.</param>
    /// <exception cref="System.ArgumentNullException">tagName.</exception>
    void AddUpdateTagItem<T>(string variable, string tagName, string tagGroup, T? typeWitness = default);
    ...
    /// </returns>
    IObservableAsync<T?> ObserveAsyncObservable<T>(string? variable, int bit = -1);
#endif
    ...
    IObservable<T?> ObserveSampled<T>(string variable, TimeSpan sampleInterval, int bit = -1, ISequencer? scheduler = null);
    ...
    IObservableAsync<T?> ObserveSampledAsyncObservable<T>(string variable, TimeSpan sampleInterval, int bit = -1, ISequencer? scheduler = null);
#endif
    ...
    IObservable<bool> ObservePing(TimeSpan interval, bool echo = false, ISequencer? scheduler = null);
    ...
    IObservableAsync<bool> ObservePingAsyncObservable(TimeSpan interval, bool echo = false, ISequencer? scheduler = null);
#endif
}
